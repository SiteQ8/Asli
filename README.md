...
**Status: version 0.1, September 2026. Nothing is built yet.** The registry, the feed and the checker start in Phase 0, on 4 October 2026. What is here today is the full write-up of what Asli will be, with the evidence behind it.

- The site: [asli.3li.info](https://asli.3li.info/)
...
Every word on the site and in both PROJECT files comes from one file, `docs/content.js`, as an Arabic and English pair. The tests enforce that pairing, so the two languages cannot drift apart.

```sh
node --test                             # 20 checks: bilingual completeness, numbers, Arabic punctuation, demo logic
node tools/build-project-md.mjs         # regenerate PROJECT.md and PROJECT.ar.md after editing content.js
node tools/build-project-md.mjs --check # fail if either file is stale, as CI does
