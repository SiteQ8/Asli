...
Lets an assistant check a message, look up a domain, or read Kuwait's registry of
genuine official channels. No account, no key, no browser.
...
| --- | --- |
| `check_message` | Is this message impersonating a Kuwaiti official body, bank or telco? Returns listed, impersonation, official or unverified, with the findings and what to do |
...
It reports what the registry and the feed actually contain. Sender names and
hotlines are not documented yet, so it says so rather than guessing, and an
unverified sender comes back as unverified rather than as safe.
